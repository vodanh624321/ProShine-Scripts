...
-- Pokemon 1: training
-- Pokemon 2: False Swipe
-- Pokemon 3: Sleep skill
-- Pokemon 4: training
-- Pokemon 5: training
-- Pokemon 6: tmp
...
function onPathAction()
    if not isPokemonUsable(1) and isPokemonUsable(4) then
        return swapPokemon(1, 4)
    end

    if not isPokemonUsable(1) and isPokemonUsable(5) then
        return swapPokemon(1, 5)
    end

if isPokemonUsable(1) and getPokemonHealthPercent(1) >= healthToRunAt and HasCatchMove(3) ~= false and HasCatchMove(2, "False Swipe") ~= false then
        ...
        if isWildBattle() and ((isOpponentShiny() and catchShineys) or (catchNotCaught and not isAlreadyCaught())) or IsPokemonOnCaptureList() then
            if getPokemonHealthPercent(getTeamSize()) > healthToRunAt then
                if isPokemonUsable(getActivePokemonNumber()) then
                    if not isOpponentShiny() then
                        if getActivePokemonNumber() == 1 then
                            return sendPokemon(2) or sendPokemon(3)
                        end
                        if getOpponentHealthPercent() >= percentToStartThrowing then
                            if moveFS = HasCatchMove(getActivePokemonNumber(), "FS") then
                                return useMove(moveFS)
                            end

                            return weakAttack()
                        else
                            if getOpponentStatus() ~= "SLEEP" and moveSleep = HasCatchMove(getActivePokemonNumber()) then
                                return useMove(moveSleep)
                            end
                        ...
                        if getOpponentHealthPercent() >= percentToStartThrowingIfShiny then
                            if moveFS = HasCatchMove(getActivePokemonNumber(), "FS") then
                                return useMove(moveFS)
                            end
            ...
            if getPokemonHealthPercent(1) < healthToRunAt or onlyCatch then
